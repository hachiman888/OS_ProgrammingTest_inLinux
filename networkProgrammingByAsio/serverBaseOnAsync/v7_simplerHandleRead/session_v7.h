...
//v7添加了更加简洁的粘包处理方案
//使用async_read来保证读到指定的字节数之后才调用回调函数,避免复杂的多次readsome逻辑
//添加了Handle_Read_Head和Handle_Read_Msg作为读取完消息体头部和读取完消息体内容的回调函数
...
public:
    ...
    Msg_Node(char* msg,unsigned short max_len):_total_len(max_len + HEAD_LENGTH),_cur_len(0){
        _data = new char[_total_len + 1]; //+1给\0留一个位置
        memcpy(_data,&max_len,HEAD_LENGTH); //先将长度信息放进消息节点中
...
public:
    ...
    void send(char* msg,std::size_t max_len); //封装的发送接口
...
private:
    void handle_Read_Head(const boost::system::error_code& ec,
        std::size_t bytes_transfered,std::shared_ptr<Session> _self_shared); //用于处理消息体头部的回调函数
    void handle_Read_Msg(const boost::system::error_code& ec,
        std::size_t bytes_transfered,std::shared_ptr<Session> _self_shared); //读取到消息体头部指定长度的信息后，处理信息的回调函数
